...
server.registerTool(
  ...
  async ({ industry, product_type }) => {
    ...
    if (!res.ok) throw new Error(`expand_tree failed: ${res.status}`);
...
server.registerTool(
  ...
  async (args) => {
    ...
    if (!res.ok) throw new Error(`rag_search failed: ${res.status}`);
    return { content: [text(await res.json())] };
  }
);
...
server.registerTool(
  ...
  async (args) => {
    ...
    if (!res.ok) throw new Error(`rag_ingest_url failed: ${res.status}`);
    return { content: [text(await res.json())] };
  }
);
...
server.registerTool(
  ...
  async (args) => {
    ...
    if (!res.ok) throw new Error(`elasticities_estimate failed: ${res.status}`);
    return { content: [text(await res.json())] };
  }
);
